...
try {
    ...
    $ContactInfo = 'new.member@example.com';
    $MembershipID = 1;
...
try {
    // Prepare the SQL statement
    $stmt = $pdo->prepare("UPDATE Members SET FirstName = :FirstName, LastName = :LastName, ContactInfo = :ContactInfo WHERE MemberID = :id");
    // Bind the parameters
    $stmt->bindParam(':FirstName', $FirstName);
    $stmt->bindParam(':LastName', $LastName);
    $stmt->bindParam(':ContactInfo', $ContactInfo);
    ...
    $FirstName = 'Updated';
    $LastName = 'Member';
    $ContactInfo = 'updated.member@example.com';
    $id = 1;
